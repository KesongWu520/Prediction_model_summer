...
MODE = "samples"  # "samples" (recommended).
    ...
    for target in targets:
        ...
        for round_idx in range(n_rounds):
            ...
            for country in countries:
                ...
                try:
                    # quantiles=QUANTILES helps some backends create quantile components when using
                    # predict_likelihood_parameters=True. Even if not, samples mode below is sufficient.
                    model = AutoARIMA(**AUTO_ARIMA_KWARGS, quantiles=QUANTILES)
                    model.fit(train_ts)

                    if MODE == "direct_quantiles":
                        pred_ts = model.predict(n=test_weeks, predict_likelihood_parameters=True)
                    else:
                        # Most robust: sample predictions then compute quantiles from samples
                        pred_ts = model.predict(n=test_weeks, num_samples=PRED_NUM_SAMPLES)
