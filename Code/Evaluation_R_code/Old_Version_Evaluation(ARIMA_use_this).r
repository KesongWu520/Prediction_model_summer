# ============================================================
# Respiratory Disease Forecast Analysis Script (English Version)
# Forecast evaluation using scoringutils
#
# Main Outputs:
#   1) Interval Coverage Plots (overall / by target / by horizon / by location / by horizon x location)
#   2) Score summary by forecast date
#   3) Score summary by horizon
#   4) Overall summary
#
# IMPORTANT (Version-specific behavior):
#   Your scoringutils::get_coverage() returns columns:
#     - interval_range              (Nominal coverage)
#     - interval_coverage           (Empirical coverage)
#     - interval_coverage_deviation (Empirical - nominal)
#   Therefore, interval coverage plots should use:
#     x = interval_range, y = interval_coverage
#
# Also, your scoringutils version assumes `model` exists by default (by = "model"),
# so we add a constant `model` column.
# ============================================================
...
# ============================================================
OUTPUT_DIR    <- "/scratch/user/s4921048/summer_research/output_darts_arima_benchmark/"  # Directory containing forecast CSVs
TRAINING_DATA <- "/scratch/user/s4921048/summer_research/data/new_train_data.csv"            # Observed data CSV
RESULTS_DIR   <- "/scratch/user/s4921048/summer_research/evaluation_result_transformer_weather_holiday_newactivation_arima/"                # Directory to save results
...
forecast_data <- merged[, .(
  model         = MODEL_NAME,       # IMPORTANT: provide model column
...
cat("  - 1_interval_coverage_overall.png\n")
cat("  - 1_interval_coverage_by_target.png\n")
cat("  - 1_interval_coverage_by_horizon.png\n")
cat("  - 1_interval_coverage_by_location.png\n")
cat("  - 1_interval_coverage_by_horizon_location.png\n")
cat("  - 1_interval_coverage_by_horizon_location_topN.png\n")
cat("  - 2_scores_by_forecast_date.csv\n")
cat("  - 2_wis_by_forecast_date.png\n")
cat("  - 2_wis_trend_faceted.png\n")
cat("  - 3_scores_by_horizon.csv\n")
cat("  - 3_wis_by_horizon_bar.png\n")
cat("  - 3_wis_by_horizon_line.png\n")
cat("  - 3_wis_decomposition.png\n")